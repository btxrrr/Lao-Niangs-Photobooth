...
import {
  CANVAS_PRESETS, BACKGROUND_PRESETS, FRAME_PRESETS, STICKER_CATEGORIES,
...
export default function FrameStudio() {
  ...
  const runExport = useCallback(async () => {
    ...
    try {
      ...
      const hasAnimated = layers.some((l) => l.isGif)
      ...
      if (!hasAnimated) {
        ...
        const loop = (now) => {
          drawScene()
          if (now - start < DURATION_MS) requestAnimationFrame(loop)
          else mr.stop()
        }
  ...
  }, [canvasPreset, background, layers, frameId])
        ...
        {step === "edit" && canvasPreset && (
          <div style={{ display: "flex", gap: 28, flexWrap: "wrap", alignItems: "flex-start" }}>
            ...
            <div style={{ flex: "0 0 auto" }}>
              ...
              >
                {exporting ? (exportMsg || "Working…") : (layers.some((l) => l.isGif) ? "Export as Video 🎥" : "Export as Image 🖼️")}
              </button>
            </div>
