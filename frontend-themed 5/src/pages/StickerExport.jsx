...
export default function StickerExport() {
  ...
  const [telegramOwnerUserId, setTelegramOwnerUserId] = useState("")
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      if (platform === "telegram" && telegramOwnerUserId.trim()) {
        payload.telegram_owner_user_id = Number(telegramOwnerUserId)
      }
  ...
  }, [platform, selected, packTitle, telegramOwnerUserId])
    ...
    <div className="denim-bg" style={{ minHeight: "100vh", paddingBottom: 60 }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "36px 24px" }}>
        ...
        <div className="glass-card" style={{ padding: 18, marginBottom: 24, display: "grid", gap: 14 }}>
          ...
          {platform === "telegram" && (
            <div style={{ display: "grid", gap: 6 }}>
              <label className="font-dm" style={{ color: "var(--text)", fontSize: 13, fontWeight: 700 }}>Telegram owner user id</label>
              <input
                value={telegramOwnerUserId}
                onChange={(e) => setTelegramOwnerUserId(e.target.value)}
                placeholder="Telegram numeric user id"
                inputMode="numeric"
                style={{ width: "100%", borderRadius: 12, border: "1px solid rgba(0,0,0,0.12)", padding: "12px 14px", fontSize: 14 }}
              />
              <p className="font-dm" style={{ fontSize: 12, color: "var(--text-light)" }}>
                Telegram sticker packs must be owned by a Telegram user id. The bot token stays on the server.
              </p>
            </div>
          )}
        </div>
